...
def sample_timesteps_mixture(T, bsz, alpha_left=1.0, alpha_right=1.0,
                             lam=0.5, device="cpu"):
    """
    Convex mixture of left and right power-law tails (paper §VII.C):
        q(t) = lam * (1 - tau)^alpha_left + (1 - lam) * tau^alpha_right,
    where tau = t / (T - 1). Probabilities are normalized to sum to 1.

    lam = 1.0 -> pure Left P-LoRA (low-noise emphasis).
    lam = 0.0 -> pure Right P-LoRA (high-noise emphasis).

    Returns (idx, probs[idx]) -- same signature as sample_timesteps_power,
    so importance weights `(1/T) / probs` keep the gradient unbiased.
    """
    t = torch.arange(T, dtype=torch.float32, device=device)
    tau = t / (T - 1)
    p_left = (1.0 - tau) ** alpha_left
    p_left = p_left / p_left.sum()
    p_right = tau ** alpha_right
    p_right = p_right / p_right.sum()
    probs = lam * p_left + (1.0 - lam) * p_right
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight on the left tail in q_mix = lam * q_left + (1 - lam) * q_right. "
             "lam=1.0 -> pure Left P-LoRA, lam=0.0 -> pure Right P-LoRA.",
    )
    parser.add_argument(
        "--alpha_left",
        type=float,
        default=1.0,
        help="Exponent of the left (low-noise) tail in the mixture sampler.",
    )
    parser.add_argument(
        "--alpha_right",
        type=float,
        default=1.0,
        help="Exponent of the right (high-noise) tail in the mixture sampler.",
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if not args.do_edm_style_training:
                    ...
                    timesteps, probs = sample_timesteps_mixture(
                        noise_scheduler.config.num_train_timesteps,
                        bsz,
                        alpha_left=args.alpha_left,
                        alpha_right=args.alpha_right,
                        lam=args.mixture_lambda,
