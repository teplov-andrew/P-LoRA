...
def sample_timesteps_beta(T, bsz, alpha=2.0, beta=2.0, device="cpu"):
    """
    Sample timesteps from a discrete Beta(alpha, beta) distribution over
    normalized tau = t/T, using midpoint discretization to avoid the
    tau in {0, 1} singularities that arise when alpha < 1 or beta < 1.

    Beta PDF on tau in (0, 1):
        p(tau) propto tau^(alpha - 1) * (1 - tau)^(beta - 1)

    Special cases recover the paper's power-law family:
        alpha=1            -> Left P-LoRA, exponent beta-1
        beta=1             -> Right P-LoRA, exponent alpha-1
        alpha=beta=1       -> uniform (DreamBooth baseline)
        alpha=beta<1       -> U-shaped (mass at both ends, low+high noise)
        alpha=beta>1       -> center-focused (mass in the middle, medium noise)

    Returns (idx, probs[idx]) -- same signature as sample_timesteps_power,
    so the existing importance weights (1/T) / probs keep the gradient
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Alpha shape parameter of the Beta(alpha, beta) timestep sampler. "
             "alpha < 1 puts mass near tau = 0 (low noise), alpha > 1 pushes mass "
             "toward tau = 1.",
    )
    parser.add_argument(
        "--beta_beta",
        type=float,
        default=2.0,
        help="Beta shape parameter of the Beta(alpha, beta) timestep sampler. "
             "beta < 1 puts mass near tau = 1 (high noise), beta > 1 pushes mass "
             "toward tau = 0. Use alpha=beta>1 for center-focused, alpha=beta<1 "
             "for U-shaped.",
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if not args.do_edm_style_training:
                    ...
                    timesteps, probs = sample_timesteps_beta(
                    ...
                    timesteps = timesteps.long()
                    timesteps_prob_pred, timesteps_prob_prior = torch.chunk(probs, 2, dim=0)
                ...
                if args.with_prior_preservation:
                    ...
                    if weighting is not None:
                        ...
                        # WEIGHTED PRIOR LOSS
                        weights = (1 / noise_scheduler.config.num_train_timesteps) / (timesteps_prob_prior)
                        squared_errors = ((model_pred_prior.float() - target_prior.float()) ** 2).mean(dim=(1, 2, 3))
                        weighted_squared_errors = squared_errors * weights
                        prior_loss = (weighted_squared_errors).sum() / weights.sum()
                ...
                if args.snr_gamma is None:
                    if weighting is not None:
                        ...
                        # WEIGHTED LOSS
                        weights = (1 / noise_scheduler.config.num_train_timesteps) / (timesteps_prob_pred)
                        squared_errors = ((model_pred.float() - target.float()) ** 2).mean(dim=(1, 2, 3))
                        weighted_squared_errors = squared_errors * weights
                        loss = (weighted_squared_errors).sum() / weights.sum()
                ...
                if accelerator.sync_gradients:
                    ...
                    # clip_grad_norm_ returns the total norm of the parameters
                    # (before clipping). Capture it for tensorboard logging.
            ...
            # Beta-P-LoRA diagnostics: timestep distribution, importance-weight
            # statistics, and gradient norm. Critical for U-shaped Beta(0.5, 0.5)
            # where weight variance can blow up (paper §V-E warning).
            with torch.no_grad():
                w = (1.0 / noise_scheduler.config.num_train_timesteps) / probs
                logs["sampling/mean_t"] = timesteps.float().mean().item()
                logs["sampling/std_t"] = timesteps.float().std().item()
                logs["weights/mean"] = w.mean().item()
                logs["weights/max"] = w.max().item()
                logs["weights/std"] = w.std().item()
